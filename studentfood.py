...
class Meal():
    ...
    def __repr__(self):
        return "{} {}".format(self.main_meal,
                              colored("({})".format(", ".join(self.extras)), attrs=["dark"]))
        ...
        for extra in extras:
            ...
            if extra_food:
                if len(extra_food) > 0 and "&nbsp" not in extra_food:
                    meal.extras.append(extra_food.strip())
...
def main():
    ...
    parser.add_argument("-o", "--out", type=str, help="write to given file (json)")
    ...
    if args.out:
        with open(args.out, "w", newline="\n", encoding="utf-8") as f:
            json.dump(found, f, default=lambda o: o.__dict__, indent=2)
